...
DeviceSimulator::Run(const HostVrp& host_vrp, const VehicleManager& vm, int count)
{
    ...
    VehicleManager *device_vms = NULL;
    cudaMalloc((void **)&device_vms, count * sizeof(VehicleManager));
    for (int i=0; i < count; i++)
        cudaMemcpy(&device_vms[i], &vm, sizeof(VehicleManager));
    ...
    simulation_kernel<<<count, vertnum, vertnum*sizeof(int)>>>(
            device_vrp, device_vms, device_costs_ptr);
